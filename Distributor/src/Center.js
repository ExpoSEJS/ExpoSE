...
class Center {
  ...
  _canStart() {
    return (
      !this._cancelled && this._running.length < this.options.maxConcurrent
    );
  }
  ...
  _printStatus() {
    Log(
      "[" +
        this._done.length +
        " done /" +
        this._strategy.length() +
        " queued / " +
        this._running.length +
        " running / " +
        this._errors +
        " errors / " +
        this._coverage.current().loc.toFixed(2) * 100 +
        "% coverage ] ***",
    );
  }
  ...
  _pushDone(test, input, pc, alternatives, coverage, errors) {
    this._done.push({
      ...
      case_coverage: this.options.perCaseCoverage
        ? new Coverage().add(coverage).final(true)
        : undefined,
