...
if (process.argv.length >= 3) {
  ...
  console.log(
    `[+] ExpoSE ${target} concurrent: ${Config.maxConcurrent} timeout: ${Config.maxTime} per-test: ${Config.testMaxTime}`,
  );
  ...
  center
    .done((center, done, errors, coverage, stats) => {
      if (Config.jsonOut) {
      ...
      console.log("");
        ...
        console.log(
          `[+] ${JSON.stringify(item.input)}${pcPart} took ${formatSeconds(item.time)}s`,
        );
      ...
      coverage.final().forEach((d) => {
        ...
        console.log(
          `[+] ${d.file}. Coverage (Term): ${Math.round(d.terms.coverage * 100)}% Coverage (Decisions): ${Math.round(d.decisions.coverage * 100)}% Coverage (LOC): ${Math.round(d.loc.coverage * 100)}% Lines Of Code: ${d.loc.total} -*`,
        );
      ...
      console.log(
        `[+] ExpoSE Finished. ${done.length} paths, ${errors} errors`,
      );
  ...
  console.log(
    `USAGE: ${process.argv[0]} ${process.argv[1]} target (Optional: initial input)`,
  );
}
