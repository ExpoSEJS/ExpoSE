...
export default function (state, ctx, model, helpers) {
  ...
  function BuildRefinements(regex, real, string_s, is_match_s) {
    ...
    function CheckCorrect(model) {
      if (model.eval(isMatch).asConstant(model)) {
        ...
        const sym_match = regex.captures.map((cap) =>
          model.eval(cap).asConstant(model),
        );

        Log.logMid(
          `Regex sanity check ${stringify(real_match)} vs ${stringify(sym_match)}`,
        );
        const is_correct =
          real_match && !Exists(real_match, sym_match, DoesntMatch);
    ...
    const NotMatch = Z3.Check(CheckCorrect, (query, model) => {
      const not = ctx.mkNot(
        ctx.mkEq(
          string_s,
          ctx.mkString(model.eval(string_s).asConstant(model)),
        ),
      );

      return [
        new Z3.Query(query.exprs.slice(0).concat([not]), [
          CheckFixed,
          NotMatch,
        ]),
      ];
    ...
    const CheckFixed = Z3.Check(CheckCorrect, (query, model) => {
      ...
      if (!real_match) {
        Log.log(
          `WARN: Broken regex detected ${regex.ast.toString()} vs ${real} in ${model.eval(string_s).asConstant(model)}`,
        );
        return [];
      }
      ...
      const query_list = regex.captures.map((cap, idx) =>
        ctx.mkEq(ctx.mkString(real_match[idx]), cap),
      );
  ...
  function RegexpBuiltinTest(regex, string) {
    ...
    if (regex.sticky || regex.global) {
      ...
      string = model
        .get(String.prototype.substring)
        .call(string, currentLastIndex);
      if (!regex.source[0] != "^") {
    ...
    const is_match_s = ctx.mkSeqInRe(
      state.asSymbolic(string),
      regexEncoded.ast,
    );

    console.log(
      `|REGEX ENCODING| ${regex}.exec(${state.asSymbolic(string).toString()}) -> (${regexEncoded.anchoredStart ? regexEncoded.anchoredStart.toString() : ""}, ${regexEncoded.anchoredEnd ? regexEncoded.anchoredEnd.toString() : ""}) (${regexEncoded.captures.reduce((last, capture, idx) => last + (idx > 0 ? "," : "") + capture.toString(), "")})`,
    );

    EnableCaptures(regexEncoded, regex, state.asSymbolic(string));
    is_match_s.checks = BuildRefinements(
      regexEncoded,
      regex,
      state.asSymbolic(string),
      is_match_s,
    ...
    if (Config.capturesEnabled && (regex.sticky || regex.global)) {
      ...
      regex.lastIndex = new ConcolicValue(
        ...
        ctx.mkIte(
          ...
          ctx.mkAdd(
            state.asSymbolic(currentLastIndex),
            regexEncoded.captures[0].getLength(),
          ),
          ctx.mkIntVal(0),
        ),
      );
  ...
  function RegexpBuiltinExec(regex, string) {
    ...
    if (Config.capturesEnabled && result_c) {
  ...
  function RegexpBuiltinMatch(regex, string) {
    if (regex.global) {
      ...
      const rewrittenRe = new RegExp(
        regex.source,
        regex.flags.replace(/g|y/g, "") + "y",
      );
  ...
  function RegexpBuiltinReplace(regex, string, replacementString) {
    ...
    if (regex.flags.includes("g")) {
      ...
      //Global replace
      while (true) {
        const next = RegexpBuiltinReplace(
          rewrittenRe,
          string,
          replacementString,
        );
      ...
      if (next) {
        ...
        let matchSize = new ConcolicValue(
          state.getConcrete(next[0]).length,
          state.asSymbolic(next[0]).getLength(),
        );
        ...
        let lhs = model
          .get(String.prototype.substring)
          .call(string, 0, next.index);
        let rhs = model
          .get(String.prototype.substring)
          .call(string, state.binary("+", next.index, matchSize));
        ...
        if (typeof state.getConcrete(replacementString) === "function") {
          ...
          if (state.getConcrete(replacementString).search(/\$[0-9]/) != -1) {
            ...
            while ((toreplace = /\$[0-9]/.exec(remaining))) {
              ...
              finalString = state.binary(
                "+",
                finalString,
                state.binary("+", before, replaced),
              );
              remaining = remaining.substr(
                toreplace.index + toreplace[0].length,
              );
            }
  ...
  function shouldBeSymbolic(regex, string) {
    return (
      ...
      (state.isSymbolic(regex.lastIndex) || state.isSymbolic(string))
    );
  }
  ...
  model.add(
    ...
    symbolicHookRe(
      String.prototype.replace,
      (base, args) =>
        shouldBeSymbolic(args[0], base) &&
        (typeof state.getConcrete(args[1]) === "string" ||
          typeof state.getConcrete(args[1]) === "function"),
  ...
  model.add(
    ...
    symbolicHookRe(
      String.prototype.split,
      (base, args) =>
        state.isSymbolic(base) &&
        (args[0] instanceof RegExp || typeof args[0] === "string"),
      (base, args) =>
        RegexpBuiltinSplit(
          args[0] instanceof RegExp ? args[0] : new RegExp(args[0]),
          base,
        ).result,
    ),
  );
