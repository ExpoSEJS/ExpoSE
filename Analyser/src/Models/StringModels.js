...
export default function (state, ctx, model, helpers) {
  ...
  const substrModel = symbolicHook(
    ...
    (base, args) =>
      ...
      (state.isSymbolic(base) ||
        state.isSymbolic(args[0]) ||
        state.isSymbolic(args[1])),
    symbolicSubstring,
  );
    ...
    symbolicHook(
      ...
      (base, args, result) => {
        ...
        return new ConcolicValue(
          result,
          ctx.mkEq(substringPart, state.asSymbolic(args[0])),
        );
      },
    ),
  );
    ...
    symbolicHook(
      ...
      (base, args, result) => {
        function relativeIndex(i) {
          return new ConcolicValue(
            state.getConcrete(i) < 0
              ? state.getConcrete(base).length - state.getConcrete(i)
              : state.getConcrete(i),
            ctx.mkIte(
              ctx.mkLt(state.asSymbolic(i), ctx.mkIntVal(0)),
              ctx.mkSub(
                state.asSymbolic(base).getLength(),
                ctx.mkMul(state.asSymbolic(i), ctx.mkIntVal(-1)),
              ),
              state.asSymbolic(i),
            ),
          );
        ...
        const length = ctx.mkRealToInt(
          state.asSymbolic(state.binary("-", to, from)),
        );

        return new ConcolicValue(
          result,
          ctx.mkSeqSubstr(state.asSymbolic(base), startIndex, length),
        );
      },
    ),
  );
  ...
  model.add(
    ...
    symbolicHook(
      ...
      (base, args) => {
        ...
        const is_well_formed =
          typeof state.getConcrete(base) === "string" &&
          typeof state.getConcrete(args[0]) === "number";
  ...
  model.add(
    ...
    symbolicHook(
      ...
      (base, args) =>
        state.isSymbolic(base) ||
        find.call(args, (arg) => state.isSymbolic(arg)),
      (base, args, result) => {
        const arg_s_list = Array.prototype.map.call(args, (arg) =>
          state.asSymbolic(arg),
        );
        const concat_s = ctx.mkSeqConcat(
          [state.asSymbolic(base)].concat(arg_s_list),
        );
...
*/
  model.add(
    ...
    symbolicHook(
      ...
      (base, a, result) => {
        ...
        const result_s = ctx.mkApp(state.stringRepeat, [
          state.asSymbolic(base),
          ctx.mkRealToInt(num_repeats),
        ]);
  ...
  model.add(
    ...
    symbolicHook(
      ...
      (base, _a) =>
        state.isSymbolic(base) &&
        typeof state.getConcrete(base).valueOf() === "string",
      (base, _a, result) => {
