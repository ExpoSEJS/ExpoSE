...
export default function (state, ctx, model, helper) {
  ...
  model.add(Array.prototype.push, function (base, args) {
    ...
    const args_well_formed =
      state.getConcrete(base) instanceof Array &&
    ...
    if (is_symbolic && args_well_formed) {
      ...
      state.updateSymbolic(
        base,
        array.setField(oldLength, value).setLength(newLength),
      );
  ...
  model.add(
    ...
    symbolicHook(
      ...
      (base, _args) => {
        const is_symbolic =
          state.isSymbolic(base) && state.getConcrete(base) instanceof Array;
        return is_symbolic;
      },
      (base, args, result) => {
        ...
        state.pushCondition(
          ctx.mkOr(
            ctx.mkEq(
              ctx.mkSelect(state.asSymbolic(base), result_s),
              searchTarget,
            ),
        ...
        const matchInArrayBody = ctx.mkImplies(
          ...
          ctx.mkNot(
            ctx.mkEq(ctx.mkSelect(state.asSymbolic(base), i), searchTarget),
          ),
        );

        const noPriorUse = ctx.mkForAll(
          [match_func_decl_name],
          intSort,
          matchInArrayBody,
          [iLessThanResult],
        );
        ...
        state.pushCondition(
          ctx.mkImplies(ctx.mkGt(result_s, ctx.mkIntVal(-1)), noPriorUse),
          true,
        );
  ...
  model.add(Array.prototype.join, function (base, args) {
    const isSymbolicVal = Array.prototype.find.call(base, (x) =>
      state.isSymbolic(x),
    );
