...
export default function (state, ctx, models, helper) {
  if (typeof window !== "undefined") {
    models.add(
      ...
      helper.NoOp(Element.prototype.getAttribute),
    );
    models.add(
      Element.prototype.setAttribute,
      helper.NoOp(Element.prototype.setAttribute),
    );
  }

  models.add(encodeURI, function (base, args) {
    const result = new ConcolicValue(
      encodeURI.call(base, state.getConcrete(args[0])),
      state.asSymbolic(args[0]),
    );
  ...
  models.add(encodeURIComponent, function (base, args) {
    if (state.isSymbolic(args[0])) {
      ...
      const result = new ConcolicValue(
        encodeURIComponent.call(base, state.getConcrete(args[0])),
        state.asSymbolic(args[0]),
      );
