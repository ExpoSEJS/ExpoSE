...
class SymbolicExecution {
  ...
  report(sourceString) {
    ...
    console.log(
      `OUTPUT_LOAD_EVENT: !!!${this.state.inlineToSMTLib()}!!! !!!"${sourceString}"!!!`,
    );
  }
    ...
    if (typeof window !== "undefined") {
      if (
        (f.name == "appendChild" ||
          ...
          f.name == "replaceChild") &&
        args[0] &&
        (args[0].src || args[0].innerHTML.includes("src="))
      ) {
  ...
  invokeFunPre(iid, f, base, args, _isConstructor, _isMethod) {
    ...
    Log.logHigh(
      `Execute function ${ObjectHelper.asString(f)} at ${this._location(iid)}`,
    );
    ...
    Log.logMid(
      fn_model
        ? "Exec Model: " + functionName + " " + new Error().stack
        : functionName + " unmodeled",
    );
  ...
  invokeFun(iid, f, base, args, result, _isConstructor, _isMethod) {
    this.state.coverage.touch(iid);
    Log.logHigh(
      `Exit function (${ObjectHelper.asString(f)}) near ${this._location(iid)}`,
    );
  ...
  getField(iid, base, offset, _val, _isComputed, _isOpAssign, _isMethodCall) {
    ...
    if (typeof window != "undefined") {
      if (base == window.navigator) {
        if (offset == "userAgent") {
          return {
            result: Object._expose.makeSymbolic(
              offset,
    ...
    Log.logHigh(
      `Get field ${ObjectHelper.asString(base)}[${ObjectHelper.asString(offset)}] at ${this._location(iid)}`,
    );
    ...
    if (base instanceof SymbolicObject) {
      Log.logMid(
        "Potential loss of precision, cocretize offset on SymbolicObject field lookups",
      );
      return {
    ...
    const result_s = this.state.isSymbolic(base)
      ? this.state.symbolicField(
          this.state.getConcrete(base),
          this.state.asSymbolic(base),
          this.state.getConcrete(offset),
          this.state.asSymbolic(offset),
        )
      : undefined;
    const result_c =
      this.state.getConcrete(base)[this.state.getConcrete(offset)];
  ...
  putFieldPre(iid, base, offset, val, _isComputed, _isOpAssign) {
    this.state.coverage.touch(iid);
    Log.logHigh(
      `Put field ${ObjectHelper.asString(base)}[${ObjectHelper.asString(offset)}] at ${this._location(iid)}`,
    );

    if (
      this.state.getConcrete(offset) === "src" ||
  ...
  putField(iid, base, offset, val, _isComputed, _isOpAssign) {
    ...
    if (
      this.state.isSymbolic(base) &&
      this.state.getConcrete(base) instanceof Array &&
      this.state.arrayType(base) == typeof val
    ) {
      ...
      const base_s = this.state
        .asSymbolic(base)
        .setField(this.state.asSymbolic(offset), this.state.asSymbolic(val));
      ...
      if (
        ...
        this.state.getConcrete(base) instanceof Element &&
        document.contains(this.state.getConcrete(base)) &&
        offset === "innerHTML"
      ) {
  ...
  functionEnter(iid, f, _dis, _args) {
    this.state.coverage.touch(iid);
    Log.logHigh(
      `Entering ${ObjectHelper.asString(f)} near ${this._location(iid)}`,
    );
  }
  ...
  binaryPre(
    ...
    if (this.state.isWrapped(left) || this.state.isWrapped(right)) {
      ...
      const is_null =
        ...
        right_c === null;
      const is_real =
        typeof left_c == "number"
          ? Number.isFinite(left_c) && Number.isFinite(right_c)
          : true;

      //TODO: Work out how to check that boxed values are the same type
      const is_same_type =
        typeof left_c === typeof right_c ||
        (!is_null && left_c.valueOf() === right_c.valueOf());

      if (!is_same_type || !is_primative || is_null || !is_real) {
        Log.log(
          `Concretizing binary ${op} on operands of differing types. Type coercion not yet implemented symbolically. (${ObjectHelper.asString(left_c)}, ${ObjectHelper.asString(right_c)}) (${typeof left_c}, ${typeof right_c})`,
        );
        left = left_c;
        right = right_c;
      } else {
  ...
  binary(
    ...
    Log.logHigh(
      "Op " +
        op +
        " left " +
        ObjectHelper.asString(left) +
        " right " +
        ObjectHelper.asString(right) +
        " result_c " +
        ObjectHelper.asString(result_c) +
        " at " +
        this._location(iid),
    );
  ...
  conditional(iid, result) {
    ...
    if (this.state.isSymbolic(result)) {
      Log.logMid(
        `Evaluating symbolic condition ${this.state.asSymbolic(result)} at ${this._location(iid)}`,
      );
