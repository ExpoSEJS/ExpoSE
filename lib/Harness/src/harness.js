...
function Construct(base, fn) {
    ...
    var createAsClass = symbolic CreateAsClass initial false;

	//Classes need special constructors
	if (fn.toString().indexOf('class') != -1 && createAsClass) {
...
function Expand(target, depth) {
	console.log('Testing ' + target.toString() + ', JSON ' + JSON.stringify(target) + ' depth:' + depth);
