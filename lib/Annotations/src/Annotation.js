...
class Annotation {
     ...
     * assumes that parent() works and returns the immediate supertype. fine for atomics.
     */
    makeSuperType(params, dependants) {
        //suspicious of this but it works for atomics
        let parentProto = this.parent();
        //let parentProto = this.constructor == Annotation.constructor ? this.constructor : this.constructor.prototype.constructor;
        ...
        let args = [params]
        //[parentProto, params].concat(dependants.slice(parentProto.cNumDependants)); //remove dependants

        let res = mk(parentProto, args);
        return res;
    }
    ...
    generateDrop(other) {
        if (other.params().length == 0) {
            ...
            return mk(this.constructor, [newParams]);
            //Remove dependents from this: .concat(this.args()));
        }
    }
    ...
    generateAs(other) {
        if (other.params().length == 0) {
            ...
            const child = mk(this.constructor, [paramL]);
            return child;
        }
    }
...
/**
 * Returns a function which returns discard: true, result: arguments[n]
 */
function DiscardOp(r) {
    if (r == -1) {
        return function() {
          return [true]; 
        };
        ...
        return function() {
          return [true, arguments[r]];
        };
    }
}

function binOp(op, l, r, bool, res) {
    return [op === 'delete', arguments[4]];
}

Annotation.prototype.isReturned = NoOp(-1);
Annotation.prototype.getField = NoOp(2);
Annotation.prototype.setField = DiscardOp(2);
Annotation.prototype.invokedOn = NoOp(3);
Annotation.prototype.binary = binOp;
